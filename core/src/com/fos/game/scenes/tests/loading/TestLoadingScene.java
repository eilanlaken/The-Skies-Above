...
import com.fos.game.scenes.tests.engine.TestSceneParentTransform;
...
public class TestLoadingScene extends Scene {
    ...
    public void update(float delta) {
        while (!doneLoading) {
            if (assetManager.update()) {
                doneLoading = true;
                context.playScene(new TestSceneParentTransform(context));
            }
        }
    }
    ...
    private void queueAssets() {
        Map<String, Class> assetsNameTypeMap = TestSceneParentTransform.getRequiredAssetsNameTypeMap();
