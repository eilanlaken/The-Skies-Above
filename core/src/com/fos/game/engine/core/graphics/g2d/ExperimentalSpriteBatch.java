 ...
 * @author mzechner
 * @author Nathan Sweet */
public class ExperimentalSpriteBatch implements Batch {
	...
	/** Constructs a new ExperimentalSpriteBatch with a size of 1000, one buffer, and the default shader.
	 * @see ExperimentalSpriteBatch#ExperimentalSpriteBatch(int, ShaderProgram) */
	public ExperimentalSpriteBatch() {
		this(1000, null);
	}

	/** Constructs a ExperimentalSpriteBatch with one buffer and the default shader.
	 * @see ExperimentalSpriteBatch#ExperimentalSpriteBatch(int, ShaderProgram) */
	public ExperimentalSpriteBatch(int size) {
		this(size, null);
	}

	/** Constructs a new ExperimentalSpriteBatch. Sets the projection matrix to an orthographic projection with y-axis point upwards, x-axis
	 ...
	 * @param defaultShader The default shader to use. This is not owned by the ExperimentalSpriteBatch and must be disposed separately. */
	public ExperimentalSpriteBatch(int size, ShaderProgram defaultShader) {
	...
	/** Returns a new instance of the default shader used by ExperimentalSpriteBatch for GL2 when no shader is specified. */
	...
	public void begin(Shader customShader) {
		if (drawing) throw new IllegalStateException("ExperimentalSpriteBatch.end must be called before begin.");
		renderCalls = 0;

		Gdx.gl.glDepthMask(false);
		if (customShader != null) {
			customShader.bind();
			customShader.loadUniformsToGPU();
		} else {
			shader.bind();
			setupMatrices();
		}
	...
	@Override
	public void end () {
		if (!drawing) throw new IllegalStateException("ExperimentalSpriteBatch.begin must be called before end.");
		if (idx > 0) flush();
		lastTexture = null;
		drawing = false;
	...
	public void draw (Texture texture, float x, float y, float originX, float originY, float width, float height, float scaleX,
		...
		if (!drawing) throw new IllegalStateException("ExperimentalSpriteBatch.begin must be called before draw.");
