...
uniform RemappingParams albedo_map;
uniform RemappingParams normal_map;
uniform RemappingParams mra_map; // mra = metallic roughness ambient
uniform float width;
...
{
    // remappings
    vec2 uv_albedo = vec2((albedo_map.x + uv.x * width) * atlas_width_inv, (albedo_map.y + uv.y * height) * atlas_height_inv);
    vec2 uv_normal  = vec2((normal_map.x + uv.x * width) * atlas_width_inv, (normal_map.y + uv.y * height) * atlas_height_inv);
    vec2 uv_mra     = vec2((mra_map.x + uv.x * width) * atlas_width_inv, (mra_map.y + uv.y * height) * atlas_height_inv);
    // material values
    //vec3 albedo     = texture(image, uv_albedo).rgb; //pow(texture(image, uv_albedo).rgb, vec3(2.2));
    vec3 albedo     = pow(texture(image, uv_albedo).rgb, vec3(2.2)); //texture(image, uv_albedo).rgb;
    float metallic  = texture(image, uv_mra).r;
    float roughness = texture(image, uv_mra).g;
    float ao        = texture(image, uv_mra).b;
    vec3 ambient    = albedo * (1 - ao);
    //vec3 ambient    = vec3(0.20) * albedo * (1 - ao); // <- also possible

    // pbr required geometry
    vec3 N = normalize(texture(image, uv_normal).rgb * 2.0 - 1.0);
    ...
    vec3 color = max(albedo * 1.2, Lo); // <- fix
