...
import com.fos.game.engine.ecs.components.lights.ComponentDirectionalLight;
...
import com.fos.game.engine.ecs.systems.renderer.materials.instances.DiffuseMapMaterialInstance;
import com.fos.game.engine.ecs.systems.renderer.shaders.base.GameShader;

public class DiffuseMapShader extends GameShader {
    ...
    public DiffuseMapShader() {
        super(Gdx.files.internal("shaders/simple/fosDiffuseMapVertexShader.glsl").readString(),
                Gdx.files.internal("shaders/simple/fosDiffuseMapFragmentShader.glsl").readString());
        shaderProgram.bind();
    ...
    public void render(Renderable renderable) {
        ...
        DiffuseMapMaterialInstance fosMaterial = (DiffuseMapMaterialInstance) modelInstance.getMaterial(renderable);
        loadMaterialParamsToGPU(fosMaterial);
        renderable.meshPart.render(shaderProgram);
    }

    private void loadMaterialParamsToGPU(final DiffuseMapMaterialInstance fosMaterial) {
        // bind texture
    ...
    protected void registerUniformLocations() {
        ...
        uniforms_directionalLightsDirectionLocations = new int[LightingEnvironment.MAX_POINT_LIGHTS];
        uniforms_directionalLightColorLocations = new int[LightingEnvironment.MAX_POINT_LIGHTS];
        uniforms_directionalLightsIntensityLocations = new int[LightingEnvironment.MAX_POINT_LIGHTS];
        for (int i = 0; i < LightingEnvironment.MAX_DIRECTIONAL_LIGHTS; i++) {
