...
import com.fos.game.engine.utils.GameMathUtils;

import java.util.HashMap;
import java.util.Map;

@Deprecated
public class RigAnimationFactoryOld {

    public static HashMap<String, RigAnimation> getNamedAnimations(final Model model, final Map<String, BoneTPoseData> namedTPoseBonesData) {
        ...
        for (Animation gdxAnimation : model.animations) {
            ...
            final RigAnimation rigAnimation = getRigAnimationFromGdxAnimation(gdxAnimation, namedTPoseBonesData);
            namedAnimations.put(name, rigAnimation);
        }
        return namedAnimations;
    }

    private static RigAnimation getRigAnimationFromGdxAnimation(final Animation source, final Map<String, BoneTPoseData> namedTPoseBonesData) {
        Array<RigAnimation.RigKeyFrame> frames = new Array<>();
        float[] keyTimes = getKeyTimes(source);
        for (int i = 0; i < keyTimes.length; i++) {
            final float keyTime = keyTimes[i];
            final WeightedPose weightedPose = extractWeightedPose(source, keyTime, namedTPoseBonesData);
            frames.add(new RigAnimation.RigKeyFrame(keyTime, weightedPose));
        }
        return new RigAnimation(frames);
    }

    private static float[] getKeyTimes(final Animation source) {
