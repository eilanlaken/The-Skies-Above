...
import com.fos.game.screens.tests.Box2DDebugRendererTestScene4;
...
public class LoadingScreen extends Scene {
    ...
    protected void update(float delta) {
        while (!doneLoading) {
            if (assetManager.update()) {
                doneLoading = true;
                context.setScreen(new Box2DDebugRendererTestScene4(context));
            }
        }
    }
    ...
    private void queueAssets() {
        Map<String, Class> assetsNameTypeMap = Box2DDebugRendererTestScene4.getRequiredAssetsNameTypeMap();
