...
import com.fos.game.screens.tests.Box2DDebugRendererTestScene6;
...
public class LoadingScreen extends Scene {
    ...
    protected void update(float delta) {
        while (!doneLoading) {
            if (assetManager.update()) {
                doneLoading = true;
                context.setScreen(new Box2DDebugRendererTestScene6(context));
            }
        }
    }
    ...
    private void queueAssets() {
        Map<String, Class> assetsNameTypeMap = Box2DDebugRendererTestScene6.getRequiredAssetsNameTypeMap();
